...
class _BlindNavigationAppState extends State<BlindNavigationApp> {
  ...
  void initState() {
    ...
    qrDataChangedNotifier.addListener(_reloadQrData); // Listen to QR data changes
  }
  ...
  Future<void> _initializeSpeech() async {
    if (await Permission.microphone.request().isGranted) {
      ...
      if (_speechInitialized) {
        ...
        _speak("Speech recognition could not be initialized.");
      }
    } else {
      _speak("Microphone permission denied. Voice commands won’t work.");
  ...
  void _stopListeningWithFeedback() async {
    if (_isListening) {
      ...
      if (_recognizedText.isEmpty) {
        ...
        await _speak("No voice input detected. Press the microphone button to try again.");
      }
      setState(() {
        _isListening = false;
        _recognizedText = '';
  ...
  void _processVoiceCommand(String command) async {
    ...
    if (_awaitingDestination) {
      _handleDestinationCommand(command);
      return;
    }
  ...
  Future<void> _handleDestinationCommand(String destination) async {
    ...
    if (destination.contains('nearest qr code')) {
      ...
      if (nearestQr.isNotEmpty) {
        _setDestination(qrId: nearestQr);
    ...
    if (matchedQrId != null) {
      _setDestination(qrId: matchedQrId);
  ...
  void handleLocationChange(Position position) {
    if (_lastDetectedQrId != null) {
      ...
      if (distanceToLastQR > 15) {
        _speak("Warning: You are moving away from the QR path. Switching to GPS navigation.");
  ...
  void dispose() {
    ...
    _qrDisplayTimer?.cancel();
    _stopListening();
    qrDataChangedNotifier.removeListener(_reloadQrData); // Clean up listener
    super.dispose();
  }
  ...
  Future<void> _fetchAndSpeakDirections(LatLng start, LatLng end) async {
    ...
    try {
      ...
      final directions = result['directions'] ?? [];
      ...
      final importantDirections = directions.where((dir) {
        return dir is String && dir.contains(RegExp(r'turn|continue|merge|exit|left|right|roundabout', caseSensitive: false));
      }).toList();

      for (int i = 0; i < importantDirections.length; i++) {
        if (!_isNavigating) return;
        ...
        await _flutterTts.speak(refinedText);
        await completer.future;

        if (!_isNavigating) return;

        if (i < importantDirections.length - 1) {
          Completer<void> nextCompleter = Completer<void>();
          _flutterTts.setCompletionHandler(() {
      ...
      if (!_isNavigating) return;
      Completer<void> overCompleter = Completer<void>();
      _flutterTts.setCompletionHandler(() {
        overCompleter.complete();
      });
      await _flutterTts.speak("over");
      await overCompleter.future;
  ...
  String _refineDirection(String direction) {
    ...
    RegExp streetNameRegex = RegExp(r"(?:onto|on)\s+([^\s,]+(?:[\s-][^\s,]+)*)");
    Match? streetMatch = streetNameRegex.firstMatch(cleanedText);
    String? streetName = streetMatch?.group(1)?.trim();
    RegExp distanceRegex = RegExp(r"in\s+([\d.,]+)\s*(miles|kilometers|meters|feet|yards)?");
    Match? distanceMatch = distanceRegex.firstMatch(cleanedText);
    String? distance = distanceMatch?.group(1)?.trim();
    String? units = distanceMatch?.group(2)?.trim();

    String refined = cleanedText;
    if (streetName != null) {
      refined += "\nStreet: $streetName";
    }
    if (distance != null) {
  ...
  Future<void> _stopNavigation() async {
    if (_isNavigating) {
      ...
      await _flutterTts.speak("Navigation has been stopped.");
  ...
  Future<void> _setDestination({LatLng? gpsDestination, String? qrId, bool useGps = false}) async {
    ...
    setState(() {
      ...
      if (_currentPosition != null) {
        ...
        if (!useGps && (_lastDetectedQrId != null && _qrData.containsKey(_lastDetectedQrId))) {
          startQrId = _lastDetectedQrId;
          start = LatLng(
            _qrData[startQrId]['current_location'][0],
            _qrData[startQrId]['current_location'][1],
          );
        } else if (!useGps && qrId != null && _qrData.containsKey(qrId)) {
          startQrId = _findNearestQr(start!);
        }
      } else if (_initialQrPosition != null && !useGps) {
    ...
    if (start == null) {
      await _speak("No location available. Please enable GPS or scan a QR code.");
      return;
    }

    if (startQrId != null && !useGps) {
      await _speak("Starting navigation from QR: ${_qrData[startQrId]['name']}");
    } else {
      await _speak("Starting from your current GPS location.");
    }
    ...
    if (qrId != null && _qrData.containsKey(qrId) && !useGps) {
      if (startQrId != null) {
        ...
        if (qrPath.isNotEmpty) {
          ...
          await _speak("No QR path found from ${_qrData[startQrId]['name']} to ${_qrData[qrId]['name']}. Switching to GPS navigation.");
          await _fetchAndSpeakDirections(start!, _destination!);
        }
      } else {
        await _speak("Please scan a QR code to start QR-based navigation.");
        await _fetchAndSpeakDirections(start!, _destination!);
      }
    } else if (gpsDestination != null) {
      await _fetchAndSpeakDirections(start!, _destination!);
  ...
  Future<void> _navigateThroughQrPath(List<String> qrPath) async {
    ...
    while (_isNavigating && currentIndex < qrPath.length - 1) {
      ...
      LatLng nextQrPos = LatLng(
        _qrData[nextQrId]['current_location'][0],
        _qrData[nextQrId]['current_location'][1],
      );

      if (_lastDetectedQrId == qrPath[currentIndex] || (_lastDetectedQrId == null && currentIndex == 0)) {
        await _speak("Proceed to ${_qrData[nextQrId]['name']}. Scan the QR code when you arrive.");
      }
    ...
    if (_lastDetectedQrId == qrPath.last) {
    ...
    while (_isNavigating && _lastDetectedQrId != expectedQrId) {
      if (_currentPosition != null) {
        ...
        if (distanceToNextQr > 150) {
          await _speak("You are off the QR path. Switching to GPS navigation.");
          await _fetchAndSpeakDirections(
            LatLng(_currentPosition!.latitude, _currentPosition!.longitude),
            _destination!,
          );
      ...
      if (DateTime.now().difference(startTime) > timeout) {
        await _speak("No QR detected recently. Please scan the next QR or I’ll switch to GPS.");
        startTime = DateTime.now();
        await Future.delayed(const Duration(seconds: 10));
        if (_lastDetectedQrId != expectedQrId) {
          await _fetchAndSpeakDirections(
  ...
  Future<void> _reloadQrData() async {
    await _loadQrData();
    setState(() {
      _addMarkers(); // Refresh markers on the map
      buildQrGraph(); // Rebuild the QR graph
    });
  }
  ...
  void _onDrawerItemTap(String title) {
    ...
    switch (title) {
      ...
      case 'QR Location':
        ...
        break;
      case 'Admin Panel': // Updated name
        Navigator.of(context).push(
          MaterialPageRoute(
            builder: (context) => AdminPanelScreen(themeProvider: widget.themeProvider), // Updated reference
          ),
        );
        break;
      case 'Settings':
      ...
      drawer: Drawer(
        child: ListView(
          ...
          children: [
            ...
            _buildDrawerItem('Admin Panel', Icons.admin_panel_settings), // Updated name
          ],
        ),
      ),
      body: Stack(
